function masterTrials = stimuli_randomize_preload_double(stimDir, numBlocks)
...
if exist(mat_file, 'file')
    ...
    %% --- STEP 3 & 4: GENERATE BLOCKS (EACH PAIR TWICE) ---
    masterTrials = cell(1, numBlocks); 
    
    for b = 1:numBlocks
        fprintf('\n--- Generating Block %d ---\n', b);
        
        % --- CHANGE HERE: Duplicate the pairs so each appears twice per block ---
        duplicatedPairs = [rawPairs, rawPairs]; 
        n = length(duplicatedPairs);
        shuff = duplicatedPairs(randperm(n)); 
        
        maxPasses = 1000; % Increased passes because solving constraints on a double-sized list is harder
            ...
            for i = 1:n
                ...
                if badSub || badStance || badLat
                    for j = randperm(n)
                        ...
                        % Validate candidate at position i
        ...
        % Warn if the algorithm couldn't find a perfect constraint solution
        if swapsInPass > 0
            warning('Block %d reached max passes without perfectly satisfying all constraints.', b);
        end
        
        % Load Images for this specific block shuffle
        fprintf('Loading images for Block %d (%d trials)...\n', b, n);
