...
%% 6. Real-Time Video Reconstruction (1-Minute Middle Slice)
[~, outName] = fileparts(xFile);
videoName = fullfile(xPath, [outName '_MiddleMinute_Sync_4.mp4']);
...
% --- CALCULATE MIDDLE MINUTE RANGE ---
midPoint = round(nPnts / 2);
halfMinuteSamples = round(srateX * 30); % 30 seconds in samples
startT = midPoint - halfMinuteSamples;
endT   = midPoint + halfMinuteSamples;
...
fprintf('Rendering 60 seconds from the middle of the trial...\n');
for t = startT:stepSize:endT
    ...
    % Marker Text Overlay
    evMatch = find([EEG.event.latency] >= t & [EEG.event.latency] < t + stepSize);
    if ~isempty(evMatch)
        mText = EEG.event(evMatch(1)).type;
        if iscell(mText); mText = mText{1}; end
        text(mean(currentPos(:,1)), mean(currentPos(:,2)), 2.0, mText, ...
            'Color', 'y', 'FontSize', 20, 'FontWeight', 'bold', 'HorizontalAlignment', 'center');
    end
    
    axis equal; grid on; view(3);
    set(gca, 'Color', 'k', 'XColor', 'w', 'YColor', 'w', 'ZColor', 'w');
    title(sprintf('Trial Time: %.2f s', xTime(t)-x_tA), 'Color', 'w');
    xlim([2, 10]);
    ylim([-2, 6]);
    zlim([0, 2.1]);
    ...
    plot(syncForceL(pIdx), 'Color', [0 0.5 1], 'LineWidth', 2.5); hold on;
    plot(syncForceR(pIdx), 'Color', [1 0 0], 'LineWidth', 2.5);
    
    if ~isempty(evMatch); xline(length(pIdx), 'g--', 'LineWidth', 2); end
    
    ylim([0, max([max(syncForceL), max(syncForceR), 200])]);
    title('Real-Time Force (N)', 'Color', 'w');
    set(gca, 'Color', 'k', 'XColor', 'w', 'YColor', 'w');
    legend({'Left', 'Right'}, 'TextColor', 'w', 'Color', 'none', 'EdgeColor', 'w', 'Location', 'northeast');
...
close(v);
fprintf('Middle minute exported: %s\n', videoName);