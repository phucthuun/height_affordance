...
4. Run `s04_dlc-data-preparation.py` locally, cell-by-cell in VS Code. Save each folder's
   corrected `CollectedData_<scorer>.h5`.
5. **Copy the relabelled project folder back to tardis.**
...
- **Paths** Root is save in `dlc_cluster_common.py` / `--data-root` /
  `$DLC_DATA_ROOT`, default `/mnt/beegfs/home/nguyen/1223-xplo-judo/10_Data/` — change this to
...
- GPU jobs (s01, s05, s06) are on 7 GPU nodes (24 GPUs total) — `--gres=gpu:1` per job is enough for a single
  subject/session/run; DLC/SuperAnimal doesn't multi-GPU out of the box.
- CPU jobs (s02, s03) are light;
