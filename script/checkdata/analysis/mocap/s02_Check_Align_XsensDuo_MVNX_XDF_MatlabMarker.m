...
mvnxLoadTime = toc; 
fprintf('>>> Success! MVNX file loaded in %.2f seconds (%.2f minutes).\n', ...
        mvnxLoadTime, mvnxLoadTime / 60);
...
if frameRate_mvnx ~= fs_lsl
    fprintf('No matching sampling rate\n');
...
%% 6. Setup Side-by-Side Dual-Panel 3D Graphics Engine
figAnim = figure('Name', 'Xsens Stream Comparison: XDF (Left) vs MVNX (Right)', ...
                 'Color', 'w', 'Position', [100 100 1400 600]);

% --- LEFT PANEL: NATIVE XDF STREAM ---
axLeft = subplot(1, 2, 1); hold(axLeft, 'on');
hBodyXdf = scatter3(axLeft, 0, 0, 0, 60, 'b', 'filled', 'MarkerFaceAlpha', 0.45, 'MarkerEdgeAlpha', 0.60);     
hComXdf  = scatter3(axLeft, 0, 0, 0, 150, 'k', 'd', 'filled', 'MarkerEdgeColor', 'w', 'LineWidth', 1.8); 
grid(axLeft, 'on'); axis(axLeft, 'equal');
xlabel(axLeft, 'X (m)'); ylabel(axLeft, 'Y (m)'); zlabel(axLeft, 'Z (m)');
title(axLeft, 'Raw LSL XDF Stream (Real-Time)', 'FontSize', 14);

% --- RIGHT PANEL: ALIGNED MVNX STREAM ---
axRight = subplot(1, 2, 2); hold(axRight, 'on');
hBodyMvnx = scatter3(axRight, 0, 0, 0, 60, 'm', 'filled', 'MarkerFaceAlpha', 0.45, 'MarkerEdgeAlpha', 0.60); % Magenta for distinction     
hComMvnx  = scatter3(axRight, 0, 0, 0, 150, 'k', 'd', 'filled', 'MarkerEdgeColor', 'w', 'LineWidth', 1.8); 
grid(axRight, 'on'); axis(axRight, 'equal');
xlabel(axRight, 'X (m)'); ylabel(axRight, 'Y (m)'); zlabel(axRight, 'Z (m)');
title(axRight, 'Aligned MVNX Stream (HD Reprocessed)', 'FontSize', 14);

% Dynamically establish bounds matching the global coordinate spaces
validIdx = ~isnan(aligned_PosData(1,:));
minVal = min(min(aligned_PosData(:, validIdx))); 
maxVal = max(max(aligned_PosData(:, validIdx)));

% Apply unified axis scaling to both panels
xlim(axLeft, [minVal maxVal]); ylim(axLeft, [minVal maxVal]); zlim(axLeft, [minVal maxVal]);
xlim(axRight, [minVal maxVal]); ylim(axRight, [minVal maxVal]); zlim(axRight, [minVal maxVal]);

% Put legends on both
legend(axLeft, [hBodyXdf, hComXdf], {'Raw Body', 'Raw CoM'}, 'Location', 'northeast');
legend(axRight, [hBodyMvnx, hComMvnx], {'HD Body', 'HD CoM'}, 'Location', 'northeast');

% Shared text overlay on the left plot for behavioral markers
txtLabel = text(axLeft, minVal + 0.1, maxVal - 0.1, maxVal - 0.1, '', ...
    'FontSize', 22, 'FontWeight', 'bold', 'Color', 'r', 'HorizontalAlignment', 'left');

% Video Export configuration Setup
[~, outName] = fileparts(fileXdf);
videoPath = fullfile(pathXdf, [outName,'_Dual_Stream_Comparison.mp4']);
...
% Set identical 3D camera viewing angles
view(axLeft, 3); view(axRight, 3);
frameStep = max(1, round(EEG.srate / v.FrameRate));

% --- ADD THIS ROW SLICING LOGIC BEFORE THE ANIMATION LOOP ---
numSegments = 23; 
posIndices = [];
for i = 1:numSegments
    startIdx = (i-1)*9 + 1; 
...
fprintf('Exporting dual-panel video from sample %d to %d...\n', startSample, endSample);

for t = startSample:frameStep:endSample
    
    % Skip completely missing buffer regions
    if isnan(EEG.data(1, t))
        continue; 
    end
    
    % --- 1. UNPACK RAW XDF DATA (From streams array) ---
    % Map the exact closest real-time XDF frame using sample t
    currentFrameXdfBody = reshape(xData_lsl(posIndices, t), 3, numSegments)';
    
    % Grab XDF native CoM at this timestamp
    [~, closestXdfComIdx] = min(abs(streams{cIdx}.time_stamps - xTime_lsl(t)));
    currentXdfCoM = double(streams{cIdx}.time_series(:, closestXdfComIdx));
    
    % --- 2. UNPACK ALIGNED MVNX DATA (From our synchronized EEG structure) ---
    currentFrameMvnxBody = reshape(EEG.data(1:(numSegments*3), t), 3, numSegments)'; 
    currentMvnxCoM = EEG.data((numSegments*3)+1 : end, t); 
    
    % Safety offsets (1mm padding to push CoM diamonds to the foreground layer)
    rOffsetZ = 0.001; 
    currentXdfCoMLayered  = [currentXdfCoM(1);  currentXdfCoM(2);  currentXdfCoM(3) + rOffsetZ];
    currentMvnxCoMLayered = [currentMvnxCoM(1); currentMvnxCoM(2); currentMvnxCoM(3) + rOffsetZ];
    
    % --- 3. INTERCEPT ACTIVE BEHAVIORAL TRIGGERS ---
    activeTrigIdx = find(t >= triggerLatencies & t <= (triggerLatencies + highlightSamples), 1);
    if ~isempty(activeTrigIdx)
        bodyColorXdf  = [1 0 0]; % Event window flash (Red)
        bodyColorMvnx = [1 0 1]; % Event window flash (Magenta-Red mix)
        currentLabel  = triggerTypes{activeTrigIdx}; 
    else
        bodyColorXdf  = [0 0 1]; % Normal (Blue)
        bodyColorMvnx = [0.5 0 0.5]; % Normal (Purple)
        currentLabel  = '';    
    end
    
    % --- 4. UPDATE GRAPHICS PIPELINE ELEMENTS SAFELY ---
    graphicHandlesValid = ishandle(hBodyXdf)  && ishandle(hComXdf) && ...
                          ishandle(hBodyMvnx) && ishandle(hComMvnx) && ishandle(txtLabel);
                      
    if graphicHandlesValid
        % Update Left Panel (XDF)
        set(hBodyXdf, 'XData', currentFrameXdfBody(:,1), 'YData', currentFrameXdfBody(:,2), ...
                      'ZData', currentFrameXdfBody(:,3), 'CData', repmat(bodyColorXdf, numSegments, 1));
        set(hComXdf,  'XData', currentXdfCoMLayered(1), 'YData', currentXdfCoMLayered(2), 'ZData', currentXdfCoMLayered(3));
        
        % Update Right Panel (MVNX)
        set(hBodyMvnx, 'XData', currentFrameMvnxBody(:,1), 'YData', currentFrameMvnxBody(:,2), ...
                       'ZData', currentFrameMvnxBody(:,3), 'CData', repmat(bodyColorMvnx, numSegments, 1));
        set(hComMvnx,  'XData', currentMvnxCoMLayered(1), 'YData', currentMvnxCoMLayered(2), 'ZData', currentMvnxCoMLayered(3));
        
        % Update Text & Capture Window
...
close(v); 
fprintf('Dual-view video rendering complete! Saved to: %s\n', videoPath);