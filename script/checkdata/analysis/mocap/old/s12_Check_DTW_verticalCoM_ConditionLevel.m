...
%
%  DYNAMIC TIME WARPING (Z-AXIS ONLY):
%  Isolates and computes structural dissimilarity based exclusively on 
%  the Z-axis (vertical/height component) of the CoM profiles.
%
...
for r = 1:maxRowsInGrid
    for c = 1:4
        ...
        if r <= length(targetColumnArray)
            ...
            % Plot time-gradient blue trajectory line
            ...
            % Plot event marker snapshot locations
            ...
            % Native Axis Formatting Calculations
            paddingValue = 0.01; 
            ...
            xlim(trialXLim); ylim(trialYLim); zlim(trialZLim);
...
%% 9. Z-Axis Dynamic Time Warping (DTW) Condition Comparison
fprintf('\nRunning Z-Axis Dynamic Time Warping Analysis...\n');
...
for groupA = 1:numConditions
    ...
    for groupB = 1:numConditions
        ...
        for i = 1:numTrialsA
            ...
            % Ensure matrix is valid and contains data frames
            if isempty(pathA) || size(pathA, 2) < 2; continue; end
            
            % CRITICAL UPDATE: Extract the 3rd row ONLY (Z-axis / Vertical tracking vector)
            zPathA = pathA(3, :);
            ...
            for j = 1:numTrialsB
                ...
                % CRITICAL UPDATE: Extract the 3rd row ONLY for target segment
                zPathB = pathB(3, :);
                
                % Standard DTW runs seamlessly on these single-dimension vectors
                [rawDist, ix, iy] = dtw(zPathA, zPathB);
...
%% 10. Visualize the Dissimilarity Heatmap
figure('Name', 'CoM_Trajectory_Z_DTW_Heatmap', 'Color', 'w', ...
...
title('Vertical CoM (Z-Axis) Trajectory Dissimilarity Matrix (DTW)', 'FontSize', 13, 'FontWeight', 'bold');
xlabel('Target Condition Group', 'FontSize', 12);
ylabel('Source Condition Group', 'FontSize', 12);

% Overlay numeric matrix weights inside tiles
...
fprintf('DTW Calculations Complete! Summary Heatmap successfully rendered.\n');in 