%% Preprocessing Script for Judo Study EEG Data
...
%% Load Configuration 
% Loads the dedicated study configuration file
run('tp_judo_bemobil_config.m');
% Clear this field because tp_judo_bemobil_import.m already handled it!
...
if ~force_recompute && exist('EEG_single_subject_final','var') && ~isempty(EEG_single_subject_final)
    ...
    % Identify boundaries using TrialOnset and TrialOffset patterns
    ...
    for idx = 1:length(allevents)
        ...
        % Filter for TrialOnset followed by integers
        ...
        % Filter for TrialOffset followed by integers
    ...
    % Fallback protocol if markers are missing
    ...
    % Define safe cutting windows with a 1-second buffer window
    ...
    % Save data diagnostics map matching BIDS naming prefix
    print(gcf, fullfile(input_filepath, [bids_base_string '_raw-full_EEG.png']), '-dpng')
    close

    % Reject boundary segments
    EEG = eeg_eegrej(EEG, removeindices);   
    
    %% Processing wrappers for basic processing and AMICA
    
    % Process baseline cleanups, filtering, and interpolation wrapper
    % Note: Subject field passed as a dummy/index or unique ID string depending on pipeline config requirements
	[ALLEEG, EEG_preprocessed, CURRENTSET] = bemobil_process_all_EEG_preprocessing(participantID, bemobil_config, ALLEEG, EEG, CURRENTSET, force_recompute);

    % Run spatial decomposition pipeline via AMICA
	bemobil_process_all_AMICA(ALLEEG, EEG_preprocessed, CURRENTSET, participantID, bemobil_config, force_recompute);
end
