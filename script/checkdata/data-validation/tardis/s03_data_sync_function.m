function s03_data_sync_function(subLabel, sesLabel, runID_in, taskName, preOffsetRed_in, baseLoc, derivativesLoc)
    % Headless HPC Multi-Modal Synchronization & Slicing Pipeline
    % Usage: run_sync_pipeline('MH9HXJ', 'S001', '001', 'heightaffordance', '3.0', '/path/to/sourcedata', '/path/to/derivatives')
    addpath(genpath('/mnt/beegfs/home/nguyen/matlab/toolbox/EEGLAB/eeglab2026.0.0/plugins/xdfimport1.2'));
    ...
    subLabel = regexprep(subLabel, '^sub-', '');
    sesLabel = regexprep(sesLabel, '^ses-', '');
    subID = ['sub-' subLabel]; 
    sesID = ['ses-' sesLabel];
    ...
    MOCAP_DIR      = fullfile(baseLoc, subID, sesID, 'motion');
    FORCE_DIR      = fullfile(baseLoc, subID, sesID, 'force');
    VIDEO_DIR      = fullfile(baseLoc, subID, sesID, 'video');
    ...
    if ~exist(fullXdfPath, 'file')
        error('Missing master timeline trace XDF: %s', fullXdfPath); 
    end
    ...
    has_video1 = ~isempty(view1Struct); 
    has_video2 = ~isempty(view2Struct);
    ...
    if ~exist(descJsonPath, 'file')
        descStruct = struct(...
            ...
            'GeneratedBy', {{struct('Name', 'MATLAB Headless Synchronizer Pipeline', 'Version', '3.2.0', ...
                            'Description', 'Unifies and slices kinematics, forces, CoM, and video feeds into BIDS derivatives.')}}, ...
            'SourceDatasets', {{struct('Description', 'Baseline tracking streams')}} ...
        );
    ...
    if isempty(mIdx) || isempty(xIdx)
        error('Required LSL Marker or Xsens Kinematic stream elements missing inside XDF.'); 
    end

    mText = streams{mIdx}.time_series(:); 
    mTime = streams{mIdx}.time_stamps(:);
    xData_lsl = double(streams{1,xIdx}.time_series); 
    xTime_lsl = streams{1,xIdx}.time_stamps;
    ...
    for i = 1:numel(mText)
        ...
        elseif contains(mText{i}, 'TrialOffset') && ~isempty(activeTrialStartTS)
            ...
            if calculatedEndTS > activeTrialStartTS
                ...
                [~, s_Sample] = min(abs(xTime_lsl - activeTrialStartTS)); 
                [~, e_Sample] = min(abs(xTime_lsl - calculatedEndTS));
                if e_Sample > totalMasterSamples; e_Sample = totalMasterSamples; end
                trials(trialCount).trial_id = trialCount; 
                trials(trialCount).start_sample = s_Sample; 
                trials(trialCount).end_sample = e_Sample;
                trials(trialCount).start_ts = activeTrialStartTS; 
                trials(trialCount).end_ts = calculatedEndTS;
            else
                warning('Skipping trial sequence: NPose to TrialOffset duration was shorter than reduction window.');
            end
            activeTrialStartTS = [];
        end
    end
    ...
    %% 3. Ingest MVNX + Loadsol & Cross-Synchronize
    hasMvnxLoadsol = true;
    try
        if isempty(mvnxDirStruct), error('MvnxLoadsol:missingFile', 'Missing MVNX trajectory file in: %s', MOCAP_DIR); end
        if isempty(forceDirStruct), error('MvnxLoadsol:missingFile', 'Missing Loadsol text log in: %s', FORCE_DIR); end
        fullMvnxPath  = fullfile(MOCAP_DIR,  mvnxDirStruct(1).name);
        fullForcePath = fullfile(FORCE_DIR, forceDirStruct(1).name);

        fprintf('Parsing MVNX structural frame elements...\n');
        tree = load_mvnx(fullMvnxPath);
        frameRate_mvnx = str2num(tree.metaData.subject_frameRate); 
        numSegments = str2num(tree.metaData.subject_segmentCount); 
        nSamples_mvnx = length(tree.frame);
        segmentNames = cell(numSegments, 1); 
        for s = 1:numSegments; segmentNames{s} = tree.segmentData(s).label; end

        mvnx_allPos = zeros(numSegments * 3, nSamples_mvnx);
        mvnx_centerOfMass = zeros(9, nSamples_mvnx);
	...
	fprintf(' -> Sync Match: HD MVNX stream trace started %.3f seconds relative to Master LSL.\n', time_lag_seconds);
        ...
        for t = 1:totalMasterSamples
            ...
            if loadsolDiff > 0.02
                master_ForceR(t) = NaN; master_ForceL(t) = NaN; 
            else
                master_ForceR(t) = loadsol.right(closestLsIdx); master_ForceL(t) = loadsol.left(closestLsIdx); 
            end
        end
        ...
        hasMvnxLoadsol = false;
        fprintf(2, '\n[WARNING] Kinematics/Loadsol failed: %s. Falling back to video-only.\n', ME.message);
    end
    ...
    for t = 1:trialCount
        ...
        if hasMvnxLoadsol
            syncTrialData = struct(...
                ...
                'center_of_mass', master_CoMData(:, sIdx:eIdx), ...
        ...
        for v = 1:numel(videoViewsToSlice)
            try
                fprintf('DEBUG: Attempting to read video: %s (exists: %d)\n', videoViewsToSlice{v}, isfile(videoViewsToSlice{v})); reader = VideoReader(videoViewsToSlice{v});
                ...
                writer = VideoWriter(outVideoPath, 'Motion JPEG AVI'); writer.FrameRate = reader.FrameRate; writer.Quality = 95; open(writer);
                ...
                fprintf('    ! Warning processing video slice [%s]: %s\n', videoAcqLabels{v}, ME.message);
            end
        end
        
        if hasMvnxLoadsol
            plotOutPath = fullfile(OUTPUT_MOTION_DIR, [baseOutName '_desc-motion-force']);
            ...
            trialCoM3D = master_CoMData(1:3, sIdx:eIdx);
            
            xlim(ax3d, [min(trialPos3D(:),[],'omitnan')-0.2, max(trialPos3D(:),[],'omitnan')+0.2]); 
            ylim(ax3d, [min(trialPos3D(:),[],'omitnan')-0.2, max(trialPos3D(:),[],'omitnan')+0.2]); 
            zlim(ax3d, [0, max(trialPos3D(:),[],'omitnan')+0.2]);
            ...
            plot(ax2d, tAxis, syncTrialData.loadsol_force_N_right, '-', 'Color', rightColor, 'LineWidth', 2.0, 'DisplayName', 'Right Foot');
            plot(ax2d, tAxis, syncTrialData.loadsol_force_N_left, '-', 'Color', leftColor,  'LineWidth', 2.0, 'DisplayName', 'Left Foot');
            ...
            vw_plot = VideoWriter(plotOutPath, 'Motion JPEG AVI'); vw_plot.FrameRate = 30; vw_plot.Quality = 90; open(vw_plot);
            frameStep = max(1, round(fs_lsl / vw_plot.FrameRate)); 
            highlightWindowSamples = round(0.5 * fs_lsl);
            ...
            for s = 1:frameStep:(eIdx - sIdx + 1)
                ...
                set(hTime, 'String', sprintf('t = %.2f s', currentTimeVal)); 
                hVline.Value = currentTimeVal;
    ...
    fprintf('\nBatch processing complete for %s, %s.\n', subID, sesID);
end