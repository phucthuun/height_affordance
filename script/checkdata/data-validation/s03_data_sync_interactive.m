...
%% 1. Ingest & Unpack Master XDF Timeline (required)
...
%% 3. Ingest MVNX + Loadsol & Cross-Synchronize (fallback-aware)
hasMvnxLoadsol = true;
try
if isempty(mvnxDirStruct), error('MvnxLoadsol:missingFile', 'Missing reprocessed Xsens trajectory file (.mvnx) inside: %s', MOCAP_DIR); end
if isempty(forceDirStruct), error('MvnxLoadsol:missingFile', 'Missing Loadsol sensor text log (.txt) inside: %s', FORCE_DIR); end
...
maxForceLimit = max([master_ForceR, master_ForceL], [], 'omitnan') * 1.05; if isempty(maxForceLimit) || ~isfinite(maxForceLimit); maxForceLimit = 1000; end
    ...
    fprintf(2, '\n!!! MVNX/Loadsol processing failed: %s\n', ME.message);

    haveFallback = exist('streams', 'var') && (has_video1 || has_video2);
    if haveFallback
        choice = questdlg( ...
...
if hasMvnxLoadsol
    ...
    fprintf('\n Batch execution finalized in VIDEO-ONLY mode (MVNX/Loadsol were unavailable). Sliced camera video exported to %s.\n', OUTPUT_VIDEO_DIR);
end