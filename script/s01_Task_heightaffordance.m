...
log = struct;
[subID, sesID, startRun, startTrial, taskLabel, PHONE_IP] = subject_info2(loc, taskLabel(1));
...
while b <= maxBlocks && ~terminateExperiment
    ...
    % Video
    experimenter_message({'Video: Python Script', '', ...
        sprintf('1. Input: Participant ID (%s), Task (%s), Session (S%s), Run (00%d)', subID, taskLabel, sesID, b),...
    ...
    experimenter_message({'LOADSOL: loadapp on tablet', '', ...
        ...
        '2. Zero     : [Measurement] Measurement >> Live chart; ask participant: Please lift your LEFT/RIGHT foot', ...
    ...
    experimenter_message({'XSENS: MVN', '', ...
    ...
    while ~calibrationComplete
        try
            fprintf('[NEON CALIBRATION] Starting calibration...\n');
            calibration_eyetracking(w1, sw, sh, ifi, log, PHONE_IP, languageInput);
